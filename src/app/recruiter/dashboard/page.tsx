...
export default async function JobInfos() {
  const recruiterId = await requireUserId();
  let jobInfos: JobDetails[] = [];

  try {
    jobInfos = await fetch(
      `${env.BACKEND_URL}/jobs/recruiter/${recruiterId}`
    ).then((res) => res.json());
  ...
  // const jobInfos = await fetch(`/api/jobs?recruiterId=${recruiterId}`).then(
  //   (res) => res.json()
  // );
  ...
  return (
    <div className="container my-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 has-hover:*:not-hover:opacity-70">
        {jobInfos.map((jobInfo: JobDetails) => (
          <Link
